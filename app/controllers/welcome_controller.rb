class WelcomeController < ApplicationController
  ...
  def test
  	response = HTTParty.get("http://api.wunderground.com/api/#{ENV['weather_api']}/geolookup/conditions/q/NC/Asheville.json")
  ...
  def index
    ...
    #checks that the state and city params are not empty before calling the API
     ...
     results = HTTParty.get("http://api.wunderground.com/api/#{ENV['weather_api']}/geolookup/conditions/q/#{params[:state]}/#{params[:city]}.json")
